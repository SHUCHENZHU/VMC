...
def create_grad_energy_update_param_fn(
   ...
   # get_position_fn_eval: GetPositionFromData[D]=None,
) -> UpdateParamFn[P, D, S]:
...
def create_kfac_update_param_fn(
    ...
    def update_param_fn(params, data, optimizer_state, key):
        ...
        params, optimizer_state, stats = optimizer.step(  
            ...
            momentum=momentum,
            damping=damping,
            #global_step_int=optimizer_state.step_counter,
        )
        data = update_data_fn(data, params)
...
def create_w2_kfac_update_param_fn(
    optimizer,
    damping: chex.Numeric,
    get_position_fn,
    ...
    if optimizer.multi_device:
        ...
        damping = utils.distribute.replicate_all_local_devices(damping)
        update_data_fn = utils.distribute.pmap(update_data_fn)
        batch_gradient_local_energy=utils.distribute.pmap(batch_gradient_local_energy)
    traced_compute_param_norm = _get_traced_compute_param_norm(optimizer.multi_device)
    ...
    def update_param_fn(params, data, optimizer_state, key):
        t=optimizer.iteration
        lr_V=learning_rate_schedule_V(t)
        alpha=alpha_schedule(t)
        dt=dt_schedule(t)
        key, subkey = utils.distribute.split_or_psplit_key(key, optimizer.multi_device)
        #positions=optimizer_state[-1]
        mcmc_positions=get_position_fn(data)
        
        positions=mcmc_positions # test
        dV=batch_gradient_local_energy(params, positions)  
        
        if acceleration:
            V=optimizer_state[-2]
            V=(1-alpha*lr_V)*V-lr_V*dV
            V = jnp.where(jnp.abs(V) > 1.0, 0.0, V)
            
        else:
            V=jnp.clip(-dV,-1.0,1.0)
            
       
        
        new_positions=positions+dt*V       
        batch=(mcmc_positions,positions,lax.stop_gradient(V))#new_positions  
        
        new_base_state,params,stats = optimizer.update(  
            params=params,
            state=optimizer_state,
            rng=subkey,
            batch=batch,  
        ...
        optimizer_state=(new_base_state, V,new_positions)
        
        
        
        
        data['walker_data']['position']=new_positions    

        data = update_data_fn(data, params)
        
        energy = stats["aux"]["energy"] # 
        variance = stats["aux"]["variance"]
        energy_noclip = stats["aux"]["energy_noclip"]
        variance_noclip = stats["aux"]["variance_noclip"]
        picked_stats = (energy, variance, energy_noclip, variance_noclip)
