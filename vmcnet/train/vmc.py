...
def test_vmc_loop(
    ...
    with CheckpointWriter(
        ...
        for epoch in range(start_epoch, nepochs):
            ...
            accept_ratio, data, key = walker_fn(params, data, key) #MCMC 更新操作
            ...
            metrics["accept_ratio"] = accept_ratio
            loss_list.append(metrics["energy"])
            var_list.append(metrics["variance"])
            metric_list.append(metrics)
            if jnp.isnan(metrics["energy"]).any():
                print('Loss nan')
                break
