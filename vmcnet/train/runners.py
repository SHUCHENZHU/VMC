...
def _assemble_mol_local_energy_fn_test(
...
def _assemble_mol_local_energy_fn(
    ...
    if local_energy_type == "standard":
        kinetic_fn = physics.kinetic.create_laplacian_kinetic_energy(log_psi_apply)
        #kinetic_fn=physics.kinetic.local_kinetic_energy(log_psi_apply)
        #kinetic_fn=physics.kinetic.constant(log_psi_apply)
        ei_potential_fn = physics.potential.create_electron_ion_coulomb_potential(
            ion_pos, ion_charges, softening_term=ei_softening
        )
        ee_potential_fn = physics.potential.create_electron_electron_coulomb_potential(
...
def _setup_vmc(
    ...
    (
        ...
        key,
        apply_pmap=apply_pmap,
        energy_data_val=energy_data_val #最后看情况改
    )
...
def run_molecule_acc(
    reload_config: ConfigDict,
    config: ConfigDict,
    initial_seed: int,
    logdir: str,
    dtype_to_use: jax.numpy.dtype = jnp.float32,
) -> None:
    """
    Run VMC on a molecule without using FLAGS.

    Args:
        reload_config: A dictionary containing reload-related configurations.
        config: A dictionary containing run configurations (e.g., VMC, eval, logging).
        initial_seed: The random seed for initialization.
        logdir: The directory where logs and checkpoints will be saved.
        dtype_to_use: The numerical data type to use for computations.
    """
    reload_from_checkpoint = (
        reload_config["logdir"] != train.default_config.NO_RELOAD_LOG_DIR
        and reload_config["use_checkpoint_file"]
    ...
    if reload_from_checkpoint:
        config["notes"] += f" (reloaded from {reload_config['logdir']}/{reload_config['checkpoint_relative_file_path']}"
        if reload_config.get("new_optimizer_state", False):
            config["notes"] += ", new optimizer state"
        config["notes"] += ")"
    ...
    (
        ...
        key,
        dtype=dtype_to_use,
        apply_pmap=config.get("distribute", False),
    )
    ...
    if reload_from_checkpoint:
        ...
        if reload_config.get("append", False):
            utils.io.copy_txt_stats(
                reload_config["logdir"], logdir, truncate=reload_at_epoch
            )
        ...
        if not reload_config.get("new_optimizer_state", False):
            optimizer_state = reloaded_optimizer_state
            start_epoch = reload_at_epoch
    ...
    params, optimizer_state, data, key, nans_detected = _burn_and_run_vmc(
        ...
        key,
        is_eval=False,
        is_pmapped=config.get("distribute", False),
        skip_burn=reload_from_checkpoint and not reload_config.get("reburn", False),
        start_epoch=start_epoch,
    )
    ...
    local_energies_filepath = os.path.join(eval_logdir, "local_energies.txt")
    if (
        config["eval"].get("record_local_energies", False)
        and os.path.exists(local_energies_filepath)
    ):
        _compute_and_save_energy_statistics(
            local_energies_filepath, eval_logdir, "statistics"
        )
