...
def combine_local_energy_terms(
    ...
    def local_energy_fn(params: P, x: Array, key: Optional[PRNGKey]) -> Array:
        ...
        for term in local_energy_terms[1:]:
            ...
            #local_energy_sum =  local_energy_sum + term(params, x)
        return local_energy_sum

    return local_energy_fn


def laplacian_psi_over_psi_old(
...
def laplacian_psi_over_psi(
    ...
    nparticles: Optional[int] = None,
    particle_perm: Optional[Array] = None,
) -> Array:                        #new version, the original one is above
    x_shape = x.shape
    flat_x = jnp.reshape(x, (-1,))
    n = flat_x.shape[0]
    def flattened_grad_log_psi_of_flat_x(flat_x_in):
...
def tanh_negative_grad_local_energy_and_grad_tanh_negative_grad_local_energy (
    grad_log_psi_apply,
    local_energy_fn,
    params,
    x,
    ...
    """
    Computes tanh(-nabla_x local_energy), and grad_x[ tanh(-nabla_x local_energy) ]
    """
    # Step 1: Compute the Laplacian (Δ(log|psi|)) using the provided function
#     def laplacian_flat_x(flat_x):
#         x_shape = x.shape
#         n = flat_x.shape[0]
#         eye = jnp.eye(n)
        
#         def flattened_grad_log_psi(flat_x_in):
#             grad_log_psi_out = grad_log_psi_apply(params, jnp.reshape(flat_x_in, x_shape))
#             return jnp.reshape(grad_log_psi_out, (-1,))

#         def _body_fun(i, val):
#             primal, tangent = jax.jvp(flattened_grad_log_psi, (flat_x,), (eye[i],))
#             return val + primal[i] ** 2 + tangent[i]

#         return lax.fori_loop(0, n, _body_fun, 0.0)

    # Step 2: Define a wrapper for Δ(log|psi|)
    flat_x = jnp.reshape(x, (-1,))
    grad_laplacian_flat_x=jax.grad(local_energy_fn)
    
    def clip(x):
        return jnp.clip(x, -1, 1)
    
    def tanh_negative_grad_local_energy (flat_x):
        return jnp.tanh( -grad_laplacian_flat_x(flat_x))
    out1=tanh_negative_grad_local_energy (flat_x)
    # Step 3: Compute the Laplacian of the Laplacian using jvp
    x_shape = x.shape
    n = flat_x.shape[0]
    eye = jnp.eye(n)

#     def _body_fun(i, val):
#         primal, tangent = jax.jvp(tanh_negative_grad_local_energy, (flat_x,), (eye[i],))
#         return val + tangent[i]
    #out2=estimate_divergence(tanh_negative_grad_local_energy, flat_x , h=1e-5) #lax.fori_loop(0, n, _body_fun, 0.0)
    out2=None
    return out1, out2
...
def estimate_divergence(vector_field, point, h=1e-5):

     
    num_dims = point.shape[0]
    ...
    for i in range(num_dims):
        ...
        point_plus = point.at[i].set(point[i] + h)
        point_minus = point.at[i].set(point[i] - h)
        
        
        f_plus = vector_field(point_plus)
        f_minus = vector_field(point_minus)
        
        
        partial_derivative = (f_plus[i] - f_minus[i]) / (2 * h)
