...
def plot_losses_simplified(
    ...
    """
    绘制损失曲线，仅绘制平滑后的曲线，并支持指定一条曲线为实线，其余为虚线。

    参数：
    - loss_dict: dict
        每个损失的名称和对应的值，格式为 {loss_name: loss_values}。
    - log_scale: bool, 可选
        是否对 y 轴使用对数刻度（默认为 False）。
    - figsize: tuple, 可选
        图表大小，默认值为 (10, 6)。
    - title: str, 可选
        图表标题，默认值为 "Loss Over Iterations"。
    - xlabel: str, 可选
        x 轴标签，默认值为 "Iteration"。
    - ylabel: str, 可选
        y 轴标签，默认值为 "Loss"。
    - solid_line_key: str, 可选
        需要以实线绘制的曲线的名称，默认为 None。
    - shift_value: float, 可选
        对所有 y 值加上的偏移量，默认为 0。
    - window_size: int, 可选
        平滑时使用的窗口大小，默认为 500。
    - marker: str, 可选
        标记点样式（例如 'o', 's', '^' 等，默认为 None，无标记点）。
    - markersize: float, 可选
        标记点的大小，默认值为 6。
    - xlim: tuple, 可选
        x 轴的显示范围，格式为 (xmin, xmax)，默认为 None（自动调整）。
    - ylim: tuple, 可选
        y 轴的显示范围，格式为 (ymin, ymax)，默认为 None（自动调整）。
    - save_path: str, 可选
        图表保存路径（默认为 None，不保存图表）。
    """
    def smooth_data(loss_list, window_size):
        """
        对数据进行窗口平滑处理。
        ...
        for i in range(len(loss_list)):
            ...
            end = min(len(loss_list), i + half_window + 1)
            smoothed_loss.append(np.mean(loss_list[start:end]))
    ...
    for i, (loss_name, loss_values) in enumerate(loss_dict.items()):
        ...
        smoothed_values = smooth_data(loss_values, window_size)
        
        # 根据 solid_line_key 决定线条样式
        linestyle = '-' if loss_name == solid_line_key else '--'
        plt.plot(
            np.array(smoothed_values) + shift_value, 
            label=loss_name, 
            color=color, 
            linestyle=linestyle, 
            marker=marker, 
...
def plot_losses(loss_dict, title="Loss Over Iterations", xlabel="Iteration", ylabel="Loss", save_path=False, log_scale=False, marker_size=8, smooth=False, window_size=500, shift_value=0, y_max_limit=None,shadow=True):
    """
    绘制损失曲线，可选择对损失数据进行平滑，同时绘制原始曲线，并增加平移和上界限制功能。
    
    参数：
    - loss_dict: dict, 每个损失的名称和对应的值
    - title: str, 图表标题
    - xlabel: str, x 轴标签
    - ylabel: str, y 轴标签
    - save_path: str, 保存路径（默认不保存）
    - log_scale: bool, 是否对 y 轴使用对数刻度
    - marker_size: int, 标记点大小
    - smooth: bool, 是否对损失值进行平滑
    - window_size: int, 平滑时使用的窗口大小
    - shift_value: float, 将所有 y 值加上的常数
    - y_max_limit: float, 限制 y 轴最大显示值（如果指定）
    """
    
    def ema_smoothing(loss_list, window_size=500):
        """对数据进行指数加权移动平均平滑"""
        smoothed_loss = []
        half_window = window_size // 2
        for i in range(len(loss_list)):
            # Define the range of indices for the window, ensuring the bounds stay within the list length
            start = max(0, i - half_window)
            end = min(len(loss_list), i + half_window + 1)
        
            # Compute the mean for the current window
            window = loss_list[start:end]
            smoothed_loss.append(np.mean(window))
        return smoothed_loss    

    plt.figure(figsize=(10, 6))

    colors = ['blue', 'red', 'green', 'orange', 'purple', 'brown', 'pink', 'gray', 'olive', 'cyan', 'magenta', 'yellow', 'black', 'teal', 'navy']
    markers = ['o', 'x', 's', 'd', '^', 'v', '>', '<', 'p', '*', 'h', 'H', '+', 'D', '|']

    for i, (loss_name, loss_values) in enumerate(loss_dict.items()):
        color = colors[i % len(colors)]
        marker = markers[i % len(markers)]
        
        # 将所有 y 值加上 shift_value
        shifted_values = np.array(loss_values) + shift_value
        
        # 绘制原始曲线（淡色）
        if shadow:
            plt.plot(shifted_values, label=f"{loss_name} (Original)", color=color, linestyle='--', alpha=0.4)
        
        # 如果平滑，则绘制平滑曲线
        if smooth:
            smoothed_values = ema_smoothing(shifted_values, window_size)
            plt.plot(smoothed_values, label=f"{loss_name} (Smoothed)", color=color, linestyle='-', marker=marker, markersize=marker_size)
...
def plot_loss_with_zoom(loss, zoom_start, zoom_end):
    """
    绘制损失曲线并在指定范围内进行局部放大。

    参数：
    loss (array-like): 损失值数组。
    zoom_start (int): 局部放大开始的迭代次数。
    zoom_end (int): 局部放大结束的迭代次数。
    """
    epochs = len(loss)
    ...
    # 绘制整体损失曲线
    ax1.plot(range(epochs), loss, label='Loss')
    ax1.set_xlabel('Iterations')
    ax1.set_ylabel('Loss')
    ax1.set_title('Loss Curve with Zoomed Inset')
    
    # 添加局部放大的子图
    ax2 = fig.add_axes([0.5, 0.5, 0.35, 0.35])  # [left, bottom, width, height]
    ax2.plot(range(zoom_start, zoom_end), loss[zoom_start:zoom_end])
    ax2.set_xlim(zoom_start, zoom_end)
    ax2.set_ylim(min(loss[zoom_start:zoom_end]), max(loss[zoom_start:zoom_end]))
    ax2.set_title('Zoomed In')
    
    # 添加局部放大的矩形框
    rect = plt.Rectangle((zoom_start, min(loss[zoom_start:zoom_end])),
                         zoom_end - zoom_start,
                         max(loss[zoom_start:zoom_end]) - min(loss[zoom_start:zoom_end]),
...
def simple_burn_and_run_vmc(
    ...
    return train.vmc.test_vmc_loop(
        params,
        optimizer_state,
        data,
        run_config.nchains,
...
#     _save_git_hash(logdir)
    ...
    start_epoch = 0
    print_epoch=config.vmc.print_epoch
#     if reload_from_checkpoint:
...
#     logging.info("Saving to %s", logdir)
    ...
    params, optimizer_state, data, key, nans_detected,loss_list,var_list,metric_list = simple_burn_and_run_vmc(
        config.vmc,
    ...
    return params, optimizer_state, data, key, nans_detected,loss_list,var_list,metric_list
    
    

    if nans_detected:
#         logging.info("VMC terminated due to Nans! Aborting.")
        print("VMC terminated due to Nans! Aborting.")
        return
    else:
        #logging.info("Completed VMC! Evaluating")
        print("Completed VMC! Evaluating")
...
def _setup_vmc(
    ...
    energy_data_val=_get_energy_val_fn(
        config.vmc, config.problem, ion_pos, ion_charges, log_psi_apply
    )
    ...
    (
        ...
        key,
        apply_pmap=apply_pmap,
        energy_data_val=energy_data_val
    )
