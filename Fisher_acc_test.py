...
def plot_losses(loss_dict, title="Loss Over Iterations", xlabel="Iteration", ylabel="Loss", save_path=False, log_scale=False, marker_size=8, smooth=False, alpha=0.9):
    """
    绘制损失曲线，可选择对损失数据进行指数加权移动平均（EMA）平滑，同时绘制原始曲线。
    ...
    - alpha: float, EMA 平滑系数，0.0 < alpha < 1.0，值越大平滑越强
    """
    ...
    for i, (loss_name, loss_values) in enumerate(loss_dict.items()):
        ...
        # 绘制原始曲线（淡色）
        plt.plot(loss_values, label=f"{loss_name} (Original)", color=color, linestyle='--', alpha=0.4)
        
        # 如果平滑，则绘制平滑曲线
        if smooth:
            smoothed_values = ema_smoothing(loss_values, alpha)
